(function (global) {
  var CustomPagination = function (el, options) {
    ...
    var default_options = { next_prev_links: "yes", inupt_forward: "yes", total: 10, count: 6, changePage: function (pageNum) { } };
    if (options) {
      for (var key in options) { default_options[key] = options[key] }
    } this.options = default_options; this.current_page = 1; this.times = 1; this.init()
  }; CustomPagination.prototype = {
    init: function () {
      var that = this; this.destroy();
      var links = this.options.next_prev_links, prev_link_html = "", next_link_html = "";
      if (links && links.toLowerCase() === "yes") {
        prev_link_html = '<a class="prev-page" href="#"><</a>'; next_link_html = '<a class="next-page" href="#">></a>'
      }
      var inputs = this.options.inupt_forward, input_html = "";
      ...
      var totals = parseInt(this.options.total), counts = parseInt(this.options.count), show_pages_html = "";
      if (totals) {
        var page_li_html = "";
        if (totals > counts) {
          for (var i = 1, len1 = counts; i < len1; i++) {
            if (i === 1) {
              page_li_html += '<li class="current-page"><a href="#">' + i + "</a></li>"
            } else {
      ...
      if (totals && counts) {
        ...
        this.el.innerHTML = custom_pagination_html; this.positionCenter();
        this.bindClickPage();
        if (links && links.toLowerCase() === "yes") {
          that.addEvent(this.el.firstChild.querySelector(".prev-page"), "click", function (e) { e = e.target ? e.target : e.srcElement; if (!e.hasAttribute("disabled")) { that.prevPage() } }); that.addEvent(this.el.firstChild.querySelector(".next-page"), "click", function (e) { e = e.target ? e.target : e.srcElement; if (!e.hasAttribute("disabled")) { that.nextPage() } })
        }
        if (inputs && inputs.toLowerCase() === "yes") {
          that.addEvent(this.el.firstChild.querySelector(".go"), "click", function (e) { that.searchInput() })
        }
      } this.disablePreNext(); this.options.changePage(this.current_page)
    }, reGeneratePageLetter: function () {
      var ul = this.el.firstChild.getElementsByTagName("ul")[0]; ul.innerHTML = "";
      var end = this.current_page; var start = end - (this.options.count - 1);
      var checked = end - 1; var totals = parseInt(this.options.total); var counts = end;
      var page_li_html = "";
      if (totals > counts) {
        for (var i = start, len1 = counts; i < len1; i++) { page_li_html += "<li>" + i + "</li>" }
    ...
    reGeneratePageBigger: function () {
      ...
      if (totals > counts) {
        ...
        for (var j = end, len2 = totals; j < len2; j++) { page_li_html += "<li>" + (j + 1) + "</li>" }
        ul.innerHTML = page_li_html
      } this.positionCenter(); this.bindClickPage(); this.times++; ul.childNodes[0].click()
    }, prevPage: function () {
      ...
      if (this.options.total > this.options.count) {
        ...
        if (should_to_page < can_to_page) { this.reGeneratePageLetter() } else { pre_select.click() }
      } else { pre_select.click() }
    },
    nextPage: function () {
      var currentx_select = this.el.querySelector(".current-page");
      ...
      if (this.options.total > this.options.count) {
        ...
        if (should_to_page > can_to_page) {
          if (should_to_page === this.options.total) {
            ...
            for (var i = 0; i < arr.length; i++) {
              if (arr[i].getAttribute("class") === "ellipsis") { ellipsis = true; break }
            }
            if (!ellipsis) {
              next_select.click();
              return false
        ...
        } else { next_select.click() }
      } else { next_select.click() }
    }, destroy: function () { this.el.innerHTML = "" }, disablePreNext: function () {
      if (this.options.total && this.options.count) {
        var links = this.options.next_prev_links; if (links && links.toLowerCase() === "yes") {
          ...
          if (this.current_page === 1) {
            ...
            prev.removeAttribute("style"); prev.removeAttribute("disabled")
          } 
          ...
          if (this.current_page === this.options.total) {
            ...
            next.removeAttribute("style"); next.removeAttribute("disabled")
          }
        }
      }
    ...
    bindClickPage: function () {
      var that = this; var ul = this.el.firstChild.getElementsByTagName("ul")[0];
            ...
            that.addEvent(lis[l], "click", function (e) {
              ...
              if (new_page !== that.current_page) {
                if (new_page === that.options.total) {
                  var judgeEllipsis = li_event.previousElementSibling || li_event.previousSibling; if (judgeEllipsis.getAttribute("class") === "ellipsis") { that.clickEndPage(); return false }
                } li_event.setAttribute("class", "current-page"); var siblings = that.siblings(li_event); for (var k = 0; k < siblings.length; k++) { siblings[k].removeAttribute("class") } that.current_page = new_page; that.options.changePage(that.current_page); that.disablePreNext()
              }
            })
          })(l)
        }
    ...
    clickEndPage: function () {
      ...
      var page_count = 0;
      if (remainder === 1) { times -= 1; page_count = counts }
      else {
        if (remainder === 0) { page_count = counts - 1 }
        else { page_count = remainder }
      }
      var ul = this.el.firstChild.getElementsByTagName("ul")[0];
      ul.innerHTML = "";
      var page_li_html = "";
      for (var j = totals - page_count, len2 = totals; j < len2; j++) { page_li_html += "<li>" + (j + 1) + "</li>" }
      ul.innerHTML = page_li_html; this.positionCenter();
      this.bindClickPage();
      this.times = times;
    ...
    searchInput: function () {
      ...
      if (input_val) {
        ...
        if (input_val < 0 || input_val === 0) {
          ...
          if (input_val > this.options.total) { input_val = this.options.total }
        }
          ...
          if (total === input_val) {
            ...
            if (t_times < all_times) {
              if (t_times === c_times) {
                ...
                this.positionCenter(); this.bindClickPage(); this.times = t_times; ul.childNodes[t_remainder - 1].click()
              }
            } else {
              var start1 = (t_times - 1) * (count - 1);
              for (var j = start1, len2 = total; j < len2; j++) { page_li_html += "<li>" + (j + 1) + "</li>" }
              ul.innerHTML = page_li_html;
              this.positionCenter();
              this.bindClickPage();
              this.times = t_times;
        ...
        } else { if (this.current_page !== input_val) { ul.childNodes[input_val - 1].click() } } input.value = ""
      }
    },
    addEvent: function (elem, type, fn) {
      ...
      for (var i = 0, elseLil = elseLi.length; i < elseLil; i++) {
        if (elseLi[i] !== currentNode && elseLi[i].getAttribute("class") !== "ellipsis") { siblings.push(elseLi[i]) }
      } return siblings
    }
